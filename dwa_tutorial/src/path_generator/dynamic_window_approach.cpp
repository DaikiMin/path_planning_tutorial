...
namespace PathPlan {
    ...
    void DynamicWindowApproach::makePathsList( const DWAParameters& dwa_param, std::vector< EvaluatedPath >* paths_list ) {
        ...
        MinMaxValue mmv;
        bool is_predict;
        double tgt_ang = data_.target_angle;
    ...
    void DynamicWindowApproach::evaluatePath ( const double dist_score, const double vel, const Path& base_path, EvaluatedValue* score, MinMaxValue* mmv ) {
        ...
        double vec_tgt2next_x = data_.target.x - base_path.next_x;
        double vec_tgt2next_y = data_.target.y - base_path.next_y;
        ...
        double diff_ang = std::fabs( data_.target_angle - base_path.next_theta );
    ...
    bool DynamicWindowApproach::generatePath2Target ( const geometry_msgs::Twist& curt_vel, const dwa_tutorial::DWACalculationData& data, geometry_msgs::Twist* output_path ) {
        DWAParameters dwa_param;
        if( !setDWAParameters( curt_vel, data.target_distance,  &dwa_param )) return false;
